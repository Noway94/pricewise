...
export async function GET (){
    try {
        ...
        //1 scrape latest product details & updated DB

        const upatedProducts= await Promise.all(
            products.map(async (currentProduct)=>{
               ...
               const product={
                ...
                //2 check each products details and send email accordinly
        ...
        return NextResponse.json({
            message:'OK',data:upatedProducts
        })
    } catch (error) {
